...
from ssb_tbmd_apis.imports.dtype_mapping import dtypes_datadok_to_pandas, intwidth_to_pandas_dtype


def test_text_mapping():
    df = pd.DataFrame(
        {"Datatype": ["Tekst"], "Length": ["10"]},
        index=["mycol"]
    )
